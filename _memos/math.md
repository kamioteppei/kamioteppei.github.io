...
単語の用法(１文でまとめないと、理解できない)

- 関数y=f(x)を微分(動詞)して、導関数f'(x)を求めた時、f'(a)とすれば、f(x)のx=aにおける微分係数(=接線の傾き)が求まる。
- 関数y=f(x)の微分(名詞)は、f'(x)であり、f'(a)とすれば、f(x)のx=aにおける微分係数(=接線の傾き)が求まる。
...
- 関数z=f(x,y)をxについて偏微分(動詞)して、偏導関数 $$ f_x(x,y) $$ を求めた時、変数xのみの関数z=g(x)の偏導関数g'(a)に置き換えられるので、、g(x)のx=aにおける偏微分係数(=接線の傾き)が求まる。
- 関数z=f(x,y)のxについての偏微分(名詞) $$ f_x(x,y) $$ は、変数xのみの関数z=g(x)の偏導関数g'(a)に置き換えられるので、、g(x)のx=aにおける偏微分係数(=接線の傾き)が求まる。
...
ただし、全てのxでの傾きを調べる必要はない。導関数は、元の式の増減を表す式だから、導関数y=f'(x)のy=0のxが、
増減の変化率=0の時、山の頂点か谷の底だとわかる。

ここら辺を理解する為に、極限値や増減表や、グラフの書き方を学習したい。
